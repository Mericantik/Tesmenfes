...
class Database:
    ...
    mycol = mydb['user']
    def __init__(self, user_id: int, bot_id: int = 0):
        from menfess import main_bot
        bot_id = main_bot if bot_id == 0 else bot_id
        if bot_id != main_bot:
            self.mycol = mydb[f'user_{bot_id}']
        self.user_id = user_id

    async def cek_user(self):
    ...
    async def hapus_pelanggan(self, user_id: int):
        self.mycol.delete_one({'_id': user_id})
        return
    ...
    async def transfer_coin(self, ditranfer: int, diterima: int, coin_awal_target_full: int, id_target: int):
        ...
        a = self.mycol.update_one(
            {"coin": coin_awal_user},
            {"$set": {
                "coin": f"{ditranfer}_{self.user_id}"
            }}
        )
        b = self.mycol.update_one(
    ...
    async def update_admin(self, id_admin: int, id_bot: int):
        ...
        self.mycol.update_one(last_data, {"$set": {"admin": data}})
    ...
    async def banned_user(self, id_banned: int, id_bot: int, alasan: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"ban": new_data}})
    ...
    async def tambah_talent(self, id_talent: int, id_bot: int, nama: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"talent": new_data}})
    ...
    async def tambah_sugar_daddy(self, id_talent: int, id_bot: int, nama: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"daddy_sugar": new_data}})
    ...
    async def tambah_pro_player(self, id_talent: int, id_bot: int, nama: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"proplayer": new_data}})
    ...
    async def tambah_teman_curhat(self, id_talent: int, id_bot: int, nama: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"temancurhat": new_data}})
    ...
    async def tambah_gf_rent(self, id_talent: int, id_bot: int, nama: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"gfrent": new_data}})
    ...
    async def tambah_bf_rent(self, id_talent: int, id_bot: int, nama: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"bfrent": new_data}})
    ...
    async def bot_handler(self, status: str):
        ...
        self.mycol.update_one(last_data, {"$set": {"bot_status": bot_status}})
    ...
    async def photo_handler(self, status: str, id_bot: int):
        ...
        self.mycol.update_one(last_data, photo_status)
    ...
    def get_pelanggan(self):
        ...
        return get_pelanggan(user_id, self.mycol)
    ...
    def get_data_bot(self, id_bot):
        found = self.mycol.find_one({'_id': id_bot})
        return data_bot(found)


class get_pelanggan():
    mycol: object
    def __init__(self, args: list, mycol):
        self.mycol = mycol
        args.remove(args[0])
    ...
    def get_data_pelanggan(self, index: int = 0):
        found = self.mycol.find_one({'_id': self.id_pelanggan[index]})
        if found:
            return data_pelanggan(found)
        else:
